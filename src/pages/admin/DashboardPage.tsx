...
function resolvePresetRange(preset: RangePreset, customStart?: string, customEnd?: string): PeriodRange {
  ...
  switch (preset) {
    case 'week':      return { start: toIsoDate(startOfWeek(today, { weekStartsOn: 1 })), end: toIsoDate(endOfWeek(today, { weekStartsOn: 1 })) }
    case 'lastWeek':  { const lw = subWeeks(today, 1); return { start: toIsoDate(startOfWeek(lw, { weekStartsOn: 1 })), end: toIsoDate(endOfWeek(lw, { weekStartsOn: 1 })) } }
    case 'month':     return { start: toIsoDate(startOfMonth(today)), end: toIsoDate(endOfMonth(today)) }
    case 'lastMonth': { const lm = subMonths(today, 1); return { start: toIsoDate(startOfMonth(lm)), end: toIsoDate(endOfMonth(lm)) } }
    case 'last14':    return { start: toIsoDate(subDays(today, 13)), end: toIsoDate(today) }
    case 'custom':    return clampRange({ start: customStart || toIsoDate(subDays(today, 6)), end: customEnd || toIsoDate(today) })
  }
}
...
function dateListFromRange(range: PeriodRange) {
  ...
  return eachDayOfInterval({ start: parseISO(safe.start), end: parseISO(safe.end) }).map(d => format(d, 'yyyy-MM-dd'))
}
...
function prepareComparison(rows: StatsComparison[], periodA: PeriodRange, periodB: PeriodRange) {
  ...
  const chartData = Array.from({ length: len }, (_, i) => {
    const dayA = aDays[i]; const dayB = bDays[i]
    ...
    return {
      ...
      periodoAData: dayA, periodoBData: dayB,
  ...
  aDays.forEach(day => { const r = mapA.get(day); if (r) { totalsA.confirmadas += r.confirmadas ?? 0; totalsA.concluidas += r.concluidas ?? 0; totalsA.canceladas += r.canceladas ?? 0; totalsA.faltas += r.faltas ?? 0 } })
  bDays.forEach(day => { const r = mapB.get(day); if (r) { totalsB.confirmadas += r.confirmadas ?? 0; totalsB.concluidas += r.concluidas ?? 0; totalsB.canceladas += r.canceladas ?? 0; totalsB.faltas += r.faltas ?? 0 } })
...
export default function DashboardPage() {
  ...
  const [form, setForm] = useState<ComparisonForm>(() => {
    ...
    return { periodAType: 'month', periodBType: 'lastMonth', periodAStart: defaultA.start, periodAEnd: defaultA.end, periodBStart: defaultB.start, periodBEnd: defaultB.end, barberId: loggedBarberId }
  })
  const [applied, setApplied] = useState<ComparisonForm>(form)

  const periodA = useMemo(() => resolvePresetRange(applied.periodAType, applied.periodAStart, applied.periodAEnd), [applied.periodAType, applied.periodAStart, applied.periodAEnd])
  const periodB = useMemo(() => resolvePresetRange(applied.periodBType, applied.periodBStart, applied.periodBEnd), [applied.periodBType, applied.periodBStart, applied.periodBEnd])
  ...
  const { data: compRes, isLoading: compLoading } = useQuery({
    ...
    queryFn: () => dashboardApi.comparison({ periodA_start: periodA.start, periodA_end: periodA.end, periodB_start: periodB.start, periodB_end: periodB.end, barbeiro_id: effectiveBarberId }),
  })
  ...
  const todayTotals = useMemo(() => todayStats.reduce(
    (acc: any, bs: any) => ({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link to={ROUTES.ADMIN_RESERVATIONS} className="btn-primary text-sm"><Plus size={16} /> Nova reserva</Link>
      </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="bg-white rounded-2xl border border-blue-100 p-4 shadow-sm flex items-center gap-3">
              ...
              <div><p className="text-xs text-gray-500">Confirmadas</p><p className="text-2xl font-bold text-gray-900">{todayTotals.confirmadas}</p></div>
            </div>
            <div className="bg-white rounded-2xl border border-emerald-100 p-4 shadow-sm flex items-center gap-3">
              <CheckCircle2 size={20} className="text-emerald-500 flex-shrink-0" />
              <div><p className="text-xs text-gray-500">Concluídas</p><p className="text-2xl font-bold text-gray-900">{todayTotals.concluidas}</p></div>
            </div>
            <div className="bg-white rounded-2xl border border-red-100 p-4 shadow-sm flex items-center gap-3">
              <XCircle size={20} className="text-red-400 flex-shrink-0" />
              <div><p className="text-xs text-gray-500">Canceladas</p><p className="text-2xl font-bold text-gray-900">{todayTotals.canceladas}</p></div>
            </div>
            <div className="bg-white rounded-2xl border border-amber-100 p-4 shadow-sm flex items-center gap-3">
              <AlertCircle size={20} className="text-amber-400 flex-shrink-0" />
              <div><p className="text-xs text-gray-500">Faltas</p><p className="text-2xl font-bold text-gray-900">{todayTotals.faltas}</p></div>
            </div>
          </div>
        </div>
      )}
            ...
            {todayStats.map((bs: any) => (
              <div key={bs.barbeiro_id} className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="flex items-center gap-1.5"><Clock size={13} className="text-blue-500" /><span className="text-xs text-gray-500">Confirmadas</span><span className="ml-auto text-sm font-bold text-gray-800">{bs.confirmadas}</span></div>
                  <div className="flex items-center gap-1.5"><CheckCircle2 size={13} className="text-emerald-500" /><span className="text-xs text-gray-500">Concluídas</span><span className="ml-auto text-sm font-bold text-gray-800">{bs.concluidas}</span></div>
                  <div className="flex items-center gap-1.5"><XCircle size={13} className="text-red-400" /><span className="text-xs text-gray-500">Canceladas</span><span className="ml-auto text-sm font-bold text-gray-800">{bs.canceladas}</span></div>
                  <div className="flex items-center gap-1.5"><AlertCircle size={13} className="text-amber-400" /><span className="text-xs text-gray-500">Faltas</span><span className="ml-auto text-sm font-bold text-gray-800">{bs.faltas}</span></div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="rounded-2xl border border-gray-100 bg-gray-50 p-3 grid gap-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 items-end">
              <div>
                ...
                <select className="input text-sm w-full" value={form.periodAType} onChange={e => updatePeriodType('A', e.target.value as RangePreset)}>
                  {presetOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
              </div>
              <div>
                <label className="label text-xs">De</label>
                <input type="date" className="input text-sm w-full" value={form.periodAStart} onChange={e => setForm(prev => ({ ...prev, periodAType: 'custom', periodAStart: e.target.value }))} />
              </div>
              <div>
                <label className="label text-xs">Até</label>
                <input type="date" className="input text-sm w-full" value={form.periodAEnd} onChange={e => setForm(prev => ({ ...prev, periodAType: 'custom', periodAEnd: e.target.value }))} />
              </div>
              <div>
                <label className="label text-xs">Barbeiro</label>
                <select className="input text-sm w-full" value={loggedBarberId ?? form.barberId ?? ''} onChange={e => setForm(prev => ({ ...prev, barberId: e.target.value ? Number(e.target.value) : undefined }))} disabled={!!loggedBarberId}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 items-end">
              <div>
                ...
                <select className="input text-sm w-full" value={form.periodBType} onChange={e => updatePeriodType('B', e.target.value as RangePreset)}>
                  {presetOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
              </div>
              <div>
                <label className="label text-xs">De</label>
                <input type="date" className="input text-sm w-full" value={form.periodBStart} onChange={e => setForm(prev => ({ ...prev, periodBType: 'custom', periodBStart: e.target.value }))} />
              </div>
              <div>
                <label className="label text-xs">Até</label>
                <input type="date" className="input text-sm w-full" value={form.periodBEnd} onChange={e => setForm(prev => ({ ...prev, periodBType: 'custom', periodBEnd: e.target.value }))} />
              </div>
              <button className="btn-primary h-10" onClick={() => setApplied(form)}><BarChart3 size={14} /> Comparar</button>
            </div>
          </div>
        </div>
            ...
            return (
              <div key={key} className={`rounded-xl border bg-white p-3 ${color}`}>
                ...
                <div className="mt-2 flex items-end justify-between">
                  <div><p className="text-xl font-bold text-gray-900">{aVal}</p><p className="text-[11px] text-gray-500">{formatPeriodLabel(applied.periodAType)}</p></div>
                  <div className="text-right"><p className="text-lg font-semibold text-gray-700">{bVal}</p><p className="text-[11px] text-gray-500">{formatPeriodLabel(applied.periodBType)}</p></div>
                </div>
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={chartData} margin={{ top: 8, right: 16, left: -20, bottom: 16 }}>
              ...
              <Tooltip
                ...
                labelFormatter={(_, payload) => { const row = payload?.[0]?.payload; if (!row) return ''; return `A: ${row.periodoAData ?? '—'} | B: ${row.periodoBData ?? '—'}` }}
              />
      ...
      <Card padding="none">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
          ...
          <Link to={ROUTES.ADMIN_RESERVATIONS} className="flex items-center gap-1 text-xs font-medium text-brand-600 hover:text-brand-700">
            ...
            {recent.map((r: any) => (
              <div key={r.id} className="flex items-center gap-4 px-5 py-3 hover:bg-gray-50 transition-colors">
                <div className="flex-shrink-0 w-8 h-8 bg-brand-100 rounded-xl flex items-center justify-center">
                  <span className="text-brand-700 text-xs font-semibold">{r.client_name.charAt(0).toUpperCase()}</span>
                </div>
                ...
                <div className="text-right flex-shrink-0">
                  <p className="text-xs font-medium text-gray-700">{format(parseISO(r.data_hora), 'd MMM, HH:mm', { locale: pt })}</p>
                  <StatusBadge status={r.status} />
                </div>
              </div>
            ))}
            {recent.length === 0 && <p className="text-center text-sm text-gray-500 py-8">Sem reservas recentes.</p>}
          </div>
        )}
      </Card>
    </div>
