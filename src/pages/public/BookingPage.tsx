...
export default function BookingPage() {
  ...
  // Repor rascunho da reserva após login/refresh — preserva o rascunho após redirect de login
  useEffect(() => {
  ...
  // Guardar rascunho sempre que o estado muda — NÃO limpa ao entrar na página
  useEffect(() => {
    if (!storage) return
    // Só guarda se houver alguma informação útil
    if (!booking.service && step === 1) return
  ...
  // Avanço automático para o próximo passo ao selecionar uma opção
  ...
  const handleSelectBarber = (barber: Barber | null, anyBarber: boolean) => {
    setBooking(bk => ({ ...bk, barber: anyBarber ? null : barber, anyBarber }))
  ...
  // Navegação pelos passos clicando no indicador
  const handleStepClick = (n: Step) => {
    // Só permite ir a passos já preenchidos ou ao passo atual
    if (n > step) return
    goToStep(n)
  }
  ...
  return (
      <div className="min-h-screen relative flex items-start justify-center pt-24 pb-16 px-4">
        ...
        <div className="w-full max-w-xl">
          {/* Step indicator — clicável para passos já completados */}
                ...
                <>
                  ...
                  {step === 2 && (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <button
                            onClick={() => handleSelectBarber(null, true)}
                        ...
                        {barbers.map(b => (
                            <button key={b.id}
                                    onClick={() => handleSelectBarber(b, false)}
                                    className={`flex items-center gap-3 p-4 rounded-2xl border transition-all text-left ${
